...
function setSpielByWebStatus(text, variant) {
...
if (spielbywebImportBtn) {
  spielbywebImportBtn.addEventListener('click', () => {
    ...
    setSpielByWebStatus(chrome.i18n.getMessage('importSpielByWebFetching'));
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      chrome.tabs.sendMessage(tabs[0].id, { action: 'import_spielbyweb_plays' }, (response) => {
        ...
        if (chrome.runtime.lastError || !response) {
          ...
          setSpielByWebStatus(chrome.i18n.getMessage('importErrorPrefix', [msg]), 'is-error');
          return;
        }
        ...
        if (response.success) {
          ...
          setSpielByWebStatus(parts.join(' · '), 'is-success');
        } else {
          setSpielByWebStatus(
