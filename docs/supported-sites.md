...
| Board Game Arena | Group tournament list | `https://boardgamearena.com/group?id=17757439` | Yes — `src/content/bga-tournament-scraper.js` | Auto-syncs new group tournaments to BGM; triggers Discord webhook on first detection | Scrapes DOM on /group pages, diffs vs chrome.storage, POSTs to /api/tournaments |
